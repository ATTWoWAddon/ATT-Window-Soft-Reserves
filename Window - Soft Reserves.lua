	...
	OnInit = function(self, handlers)
								...
								if guid then
									if not groupMembers[guid] then
										...
										if (((yearsOffline or 0) * 12) + (monthsOffline or 0)) < 3 or debugMode then
											...
											if a then tinsert(a.g, CreateSoftReserveUnit(guid, { parent = a, OnUpdate = app.AlwaysShowUpdate })); end
										end
									end
								end
							end
		...
		self:SetData(app.CreateRawText(self.Title, {
			...
			OnUpdate = function(data)
				...
				if not me then
					me = CreateSoftReserveUnit(app.GUID, { parent = data, priority = 8, OnUpdate = app.AlwaysShowUpdate });	-- YOU!
		...
		app.Settings.CreateInformationType(self.Title, {
			priority = 2.9,
			text = self.Title,
					...
					if reservesForItem then
						...
						for i,guid in ipairs(reservesForItem) do
							if guid and IsGUIDInGroup(guid) then
								tinsert(tooltipInfo, { left = left, right = CreateSoftReserveUnit(guid).tooltipText });
								left = nil;
							end
						end
					end
